...
if(isset($_POST['valierRDvBtn'])){
	...
	if(!empty($_POST['Confirmerdaterdv']) AND !empty($_POST['Confirmerheurerdv'])){
	    ...
	    $req=$conbd->query('SELECT * FROM rendezvous WHERE Annulerrdv = "'.$f.'" AND Etatrdv="'.$e.'" AND idRDV = "'.$AdmidRDV.'"');         
        $valeurUsername=$req->rowcount();   
		if($valeurUsername == 0){
			
	  $req=$conbd->prepare('UPDATE rendezvous SET Confirmerdaterdv="'.$Confirmerdaterdv.'" WHERE idRDV = "'.$AdmidRDV.'"');
	  $req->execute();
	  $req=$conbd->prepare('UPDATE rendezvous SET Confirmerheurerdv="'.$Confirmerheurerdv.'" WHERE idRDV = "'.$AdmidRDV.'"');
	  $req->execute();	
	  $req=$conbd->prepare('UPDATE rendezvous SET Etatrdv="'.$Etatrdv.'" WHERE idRDV = "'.$AdmidRDV.'"');
	  $req->execute();	
      if($req){
