...
if(isset($_POST['SecRajouter'])){
   ...
   $d="Demande";
	    ...
	    $reqq=$conbd->query('SELECT * FROM rendezvous WHERE Annulerrdv="'.$c.'" AND Etatrdv="'.$d.'" AND idRDV = "'.$SecdRDV.'"');         
        $valeurUsername=$reqq->rowcount();   
		if($valeurUsername == 0){
			
		$req=$conbd->prepare('UPDATE rendezvous SET Annulerrdv="'.$radiordv2.'" WHERE idRDV = "'.$SecdRDV.'"');
		$req->execute();
			  if($req){
